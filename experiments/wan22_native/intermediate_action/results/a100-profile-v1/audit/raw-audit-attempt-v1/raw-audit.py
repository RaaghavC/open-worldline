...
try:
 ...
 for index in (28,29):
  ...
  for step,row in enumerate(w['placements'][str(index)]['updates'],1):
   ...
   x,t,ident=effect.auxiliary_inputs(data['windows'],noise)
   assert ident==row['auxiliary']['input_identity'] or all(row['auxiliary']['input_identity'].get(k)==v for k,v in ident.items())
   gs=[]
   ...
   gru=math.sqrt(sum(float(np.square(v.astype(np.float64)).sum()) for n,v in grad.items() if n.startswith('command_encoder.gru.')))
   # Published parameter names are checked below; gradient gate uses exact saved rows.
   results.append(dict(placement=index,update=step,main_future_mse_fp64=sum(losses)/2,auxiliary_mse_fp64=aux,gradient_l2_fp64=norm,reported_gru_gradient=row['command_gru_gradient_l2']))
