...
The public video release is being prepared under tag `wan22-factorial-video-a100-v1`. It contains all raw evidence plus a separately downloadable comparison viewer. Large files stay in release assets. This directory retains exact small evidence and source copies, listed in [evidence-copies.json](evidence-copies.json). The final training checkpoint and complete training evidence are already available in the [training release](https://github.com/RaaghavC/open-worldline/releases/tag/wan22-factorial-training-a100-v1).
