...
class DiagnosticTests(unittest.TestCase):
    ...
    def test_all_calls_matched_inputs_order_and_effect(self):
        ...
        def adapt(cp,x,t,c,a):
            ...
            if t[0,-1]==506:return torch.full_like(x,{'0':1.,'16':2.,'128':2.5}[cp])
            return x+c[0,0]+(a[0,0,5]*c[0,0]*({'0':0.,'16':.01,'128':.02}[cp]))
        report=d.evaluate(native,adapt,cases,values,contexts,commands,lambda n,v:saved.update({n:v.clone()}),lambda:None)
        self.assertEqual(report['predictions'],20);self.assertEqual(len(calls),20);self.assertEqual(len(report['calls']),20)
        self.assertEqual(len(saved),26);self.assertEqual([r[0] for r in calls],['native']*2+['0']*6+['16']*6+['128']*6)
        for row in calls:
        ...
        self.assertEqual(report['relative_objective_improvement_128_vs16']['closed'],.75)
        self.assertEqual(report['objective']['128']['open'],.25)
    ...
    def test_exact_admission_and_parent_failure_single_use(self):
        with tempfile.TemporaryDirectory() as temporary:
            out=Path(temporary).resolve();plan={'source_sha256':{'a':'b'},'cpu_report_sha256':'c','training_audit_sha256':'e','checkpoint_128':{'checkpoint_sha256':'f'}}
            (out/'plan.json').write_text('{}')
            a={'schema':'worldline-action-fixed-input-comparison128-admission-v1','decision':'admit','issued_by':'parent-agent','plan_sha256':d.vi.sha(out/'plan.json'),'source_sha256':plan['source_sha256'],'limits':d.limits('pair'),'training_admitted':False,'generation_admitted':False,'predictions':20,'cpu_report_sha256':'c','training_audit_sha256':'e','checkpoint128_sha256':'f'}
    ...
    def test_preparation_requires_completed128_and_matching_independent_audit(self):
        with tempfile.TemporaryDirectory() as temporary:
            base=Path(temporary).resolve();prior=base/'prior';prior.mkdir();report=base/'review.json';report.write_text('{}')
            reader=Mock();reader.training.side_effect=ValueError('completed128 required')
            reference=Mock();reference.read_prepared.return_value=({}, {}, {}, {}, {}, {})
            with patch.object(d,'review',return_value='a'),patch.object(d,'reference',return_value=reference),patch.object(d,'training_reader',return_value=reader):
                with self.assertRaisesRegex(ValueError,'completed128'):
                    d.prepare(prior,base/'training',base/'audit',report,base/'incomplete')
                self.assertEqual(d.vi.read(base/'incomplete/metrics.json')['model_execution'],False)
                reader.training.side_effect=None;reader.training.return_value=({},base/'checkpoint',{})
                reader.validate_training_audit.side_effect=ValueError('actual audit required')
                with self.assertRaisesRegex(ValueError,'actual audit'):
                    d.prepare(prior,base/'training',base/'audit',report,base/'unaudited')
                self.assertFalse((base/'unaudited/checkpoint-0128.safetensors').exists())
