...
SCHEMA = 'worldline-action-cuda-final128-visual-pair-v1'
SCOPE = 'final128-matched-wait-interact-visual-pair'
...
def sources():
    ...
    local = {p:sha(HERE/p) for p in ('inputs.py','run.py','sampler.py','test_run.py','training-source.json')}
...
def training(directory):
    ...
    if (parent.get('schema')!='worldline-action-cuda-fixed128-run-v1' or parent.get('status')!='passed'
        ...
        raise ValueError('Completed fresh fixed128 spatial training required')
...
def read_prepared(directory):
    ...
    if (plan.get('schema')!=SCHEMA or plan.get('status')!='prepared' or plan.get('scope')!=SCOPE
        ...
        or plan.get('training') is not False or plan.get('final_checkpoint_updates')!=128 or plan.get('checkpoint_selection') is not False): raise ValueError('Exact current matched visual plan required')
...
def validate_training_audit(path,trained):
    ...
    if value.get('schema')!='worldline-action-cuda-fixed128-actual-independent-v1' or value.get('status')!='passed' or value.get('completed_updates')!=128 or value.get('foundation_values_unchanged') is not True or value.get('final_checkpoint_only') is not True or any(value.get('identity',{}).get(k)!=v for k,v in identity.items()):
