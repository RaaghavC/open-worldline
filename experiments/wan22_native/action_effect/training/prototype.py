...
def protocol(profile):
    ...
    return {'schema': 'worldline-action-effect128-prototype-v1', 'profile': profile,
            ...
            'loss': 'Original half each branch future-only FM, plus lambda1 guided clean-endpoint contrast MSE at start0 only; all losses exclude initial latent',
            'negative_context_adapter_training':True, 'codec_prefix_limits': numerical.PREFIX, 'native_zero_adapter_limits': numerical.PARITY,
            'training_forwards': 2*UPDATES, 'auxiliary_weight':effect.LAMBDA, 'auxiliary_endpoint_scale':1.0, 'auxiliary_updates':list(range(1,129,4)), 'auxiliary_feature_extracts':64, 'auxiliary_head_predictions':128, 'initial_native_parity_forwards': 2, 'initial_bridge_parity_forwards': 2,
...
def run_sequence(bridge, windows, schedule, draws, context, optimizer, output, *,
                 expected_initial, expected_core, native_predict, identity, negative_context,
    ...
    report = {'schema': 'worldline-action-effect128-prototype-result-v1', 'status': 'running',
              'identity': identity, 'completed_updates': 0, 'updates': [], 'schedule': schedule,
              'raw_files': {}, 'base_unchanged': None, 'resume_supported': False,
              'production_admission_implemented': False, 'quality_assessed': False, 'negative_context_adapter_training':True}
    ...
    class RetainingBridge:
        ...
        shape, tokens, profile = bridge.shape, bridge.tokens, bridge.profile
        def extract_features(self,*args,**kwargs):return bridge.extract_features(*args,**kwargs)
        def predict_from_features(self,*args,**kwargs):return bridge.predict_from_features(*args,**kwargs)
        training_calls = 0
    ...
    try:
        ...
        retain('negative', {'context': negative_context})
        retain('auxiliary-target-difference', {'difference':windows['open-0000']['target']-windows['closed-0000']['target']})
        ...
        current = effect.execute_steps(proxy, windows, schedule[:2], draws, context, optimizer,
                                            native_predict=native_predict, retain=retain, checkpoint=checkpoint,
                                            progress=progress, negative_context=negative_context, check=check, synchronize=synchronize)
        for index, row in enumerate(schedule[2:], 3):
            check(); synchronize(); start_time = time.monotonic()
            result = effect.paired_update(proxy, [windows[key] for key in row['branches']],
                                             draws[row['noise_key']], row['k'], context, optimizer, check=check,
                                             auxiliary=row['start']==0,negative_context=negative_context,retain=retain,update=index)
            synchronize()
        ...
        report.update(current, status='passed', base_unchanged=True, inputs_unchanged=True,
                      training_forwards=proxy.training_calls, auxiliary_feature_extracts=sum(r['auxiliary']['feature_extracts'] for r in current['updates']),
                      auxiliary_head_predictions=sum(r['auxiliary']['head_predictions'] for r in current['updates']),
                      main_feature_forwards=256,main_head_predictions=256,total_training_feature_forwards=320,total_training_head_predictions=384,
                      parity_native_forwards=2,parity_bridge_forwards=2)
        if report['auxiliary_feature_extracts']!=64 or report['auxiliary_head_predictions']!=128:raise RuntimeError('Exact auxiliary invocation counts differ')
        return report
