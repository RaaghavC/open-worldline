...
class Fixed128Tests(unittest.TestCase):
    def setUp(self):
    ...
    def inputs(self):
        ...
        prefix,draws=p.original.make_draws('fixed16',shape=tuple(target.shape))
        draws['rng_initial']=torch.Generator(device='cpu').manual_seed(p.original.SEED).get_state()
        initial,schedule,draws=p.fresh_inputs(tuple(target.shape),prefix_schedule=prefix,prefix_draws=draws,initial=initial)
    ...
    def run_case(self, case, output):
        ...
        return p.run_sequence(bridge, windows, schedule, draws, context, optimizer, output,
                              ...
                              identity={'fixture': True, 'profile': 'baseline', 'fresh': True},negative_context=context.flip(0).clone()+.02)

    def test_all128_actual_paired_updates_and_immutable_checkpoint_order(self):
        ...
        self.assertEqual(checkpoint_order, list(range(0,129,16)))
        self.assertEqual([row['start'] for row in report['updates']], [0, 8, 32, 49] * 32)
        self.assertEqual(report['training_forwards'], 256)
        self.assertEqual(report['auxiliary_feature_extracts'],64)
        self.assertEqual(report['auxiliary_head_predictions'],128)
        self.assertEqual(len(list(output.glob('auxiliary-*-closed-positive.safetensors'))),32)
        self.assertEqual(report['bridge_predictions'], 258)
        ...
        self.assertEqual(len(list(output.glob('prediction-*.safetensors'))), 256)
        self.assertEqual(len(list(output.glob('gradients-after-clip-*.safetensors'))), 128)
        ...
        self.assertTrue(all(float(value['step']) == 128 for value in optimizer.state.values()))
    ...
    def test_later_optimizer_failure_does_not_advance_last_valid_checkpoint(self):
        ...
        with patch.object(optimizer, 'step', side_effect=fail_eighteenth):
            with self.assertRaises(FloatingPointError):
                self.run_case(case, output)
        pointer = json.loads((output / 'last-valid.json').read_text())
        self.assertEqual(pointer['completed_updates'], 16)
        self.assertFalse((output / 'checkpoint-0018').exists())
        recovery = torch.load(output / 'checkpoint-0016/optimizer-and-rng.pt', weights_only=True)
    ...
    def test_fresh_draw_prefix_and_original_command_alignment(self):
        ...
        first16,original16=p.original.make_draws('fixed16',shape=tuple(windows['closed-0000']['target'].shape))
        self.assertEqual(rows[:16],first16)
        self.assertTrue(all(torch.equal(draws[k],v) for k,v in original16.items()))
        generator=torch.Generator(device='cpu');generator.set_state(original16['rng_after_0015'])
        self.assertEqual(rows[16]['k'],int(torch.randint(50,951,(1,),generator=generator)))
        self.assertTrue(torch.equal(draws['noise_0016'],torch.randn(windows['closed-0000']['target'].shape,generator=generator)))
