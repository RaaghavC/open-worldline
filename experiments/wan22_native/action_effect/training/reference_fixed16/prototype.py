"""Isolated fixed16 computation prototype. No loader, launcher or execution CLI."""
import argparse
import copy
import json
import math
...
def protocol(profile):
    ...
    return {'schema': 'worldline-action-cuda-fixed16-prototype-v1', 'profile': profile,
            'paired_updates': 16, 'starts': list(STARTS), 'branch_order': ['closed', 'open'],
            ...
            'training_forwards': 32, 'initial_native_parity_forwards': 2, 'initial_bridge_parity_forwards': 2,
            'completed_checkpoints': list(range(17)), 'model_execution': False,
...
def fresh_inputs(shape, *, test_configuration=None):
    """Construct a new adapter and private draws; never open a checkpoint."""
    if len(shape) != 5 or shape[0] != 1 or shape[2] != 5:
        raise ValueError('Use B=1 and exactly five latent frames')
    adapter = original.fresh_adapter(test_configuration=test_configuration)
    initial = {name: value.detach().cpu().contiguous().clone() for name, value in adapter.state_dict().items()}
    schedule, draws = original.make_draws('fixed16', shape=shape)
    draws['rng_initial'] = torch.Generator(device='cpu').manual_seed(original.SEED).get_state().clone()
    return adapter, initial, schedule, draws
...
def validate_inputs(windows, schedule, draws, shape):
    ...
    if set(windows) != keys or len(schedule) != 16:
        raise ValueError('Require all eight windows and sixteen paired updates')
    ...
    expected_draws = {'rng_initial'} | {f'{kind}_{i:04d}' for i in range(16) for kind in ('noise', 'rng_after')}
    if set(draws) != expected_draws:
        raise ValueError('Exactly sixteen saved noises and generator states are required')
...
def run_sequence(bridge, windows, schedule, draws, context, optimizer, output, *,
    ...
    checks, then the unchanged paired objective for the remaining fourteen.
    ...
    for group in optimizer.param_groups:
        if any(group.get(key) != value for key, value in original.OPTIMIZER.items()):
            raise ValueError('Keep the original fixed16 optimizer settings')
    ...
    report = {'schema': 'worldline-action-cuda-fixed16-prototype-result-v1', 'status': 'running',
    ...
    def progress(value):
        report.update(copy.deepcopy(value))
    ...
    try:
        ...
        validate_inputs(windows, schedule, draws, shape)
        if (proxy.training_calls != 32 or current['bridge_predictions'] != 34
                or current['native_reference_predictions'] != 2 or current['completed_updates'] != 16):
            raise RuntimeError('Fixed16 forward or optimizer counts differ')
