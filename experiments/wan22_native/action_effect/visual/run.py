...
def sample_arms(core,out,checkpoint,checkpoint_sha,values,contexts,commands,check):
    ...
    for arm in ('closed','open'):
        ...
        # The unchanged sampler was written for positive-only training. Its
        # hard-coded training label is superseded by this admitted family.
        record.update(negative_context_adapter_training=True,
            negative_context_training_scope='start0 auxiliary CFG contrast; main FM uses positive text only',
            training_scope_metadata_corrected_by='effect128 visual runner',
            seconds=time.monotonic()-began,commands_label='wait then 15 left turns' if arm=='closed' else 'interact then 15 left turns')
