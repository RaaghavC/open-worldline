...
class VisualTests(unittest.TestCase):
    ...
    def test_matched_private_inputs_and_all50_states(self):
        ...
        for arm in ('closed','open'):
            self.assertTrue(records[arm]['negative_context_adapter_training'])
            self.assertEqual(records[arm]['negative_context_training_scope'],'start0 auxiliary CFG contrast; main FM uses positive text only')
    ...
    def test_final128_audit_binds_checkpoint_and_complete_source(self):
        ...
        report={'schema':'worldline-action-effect128-actual-independent-v1','status':'passed','completed_updates':128,'foundation_values_unchanged':True,'final_checkpoint_only':True,'auxiliary_updates':32,'auxiliary_head_predictions':128,'identity':identity}
        path=self.root/'audit.json';inp.atomic(path,report);inp.validate_training_audit(path,trained)
        for key,value in [('completed_updates',127),('foundation_values_unchanged',False),('status','running'),('auxiliary_updates',31),('auxiliary_head_predictions',127)]:
    ...
    def test_training_eligibility_rejects_prior16_and_incomplete128_before_loading(self):
        ...
        with patch.object(inp,'file',side_effect=lambda b,n:Path(b)/n),patch.object(inp,'read',side_effect=lambda path:rows[str(Path(path).relative_to(base))]),patch.object(inp.sampler,'load_adapter_checkpoint') as load:
            with self.assertRaisesRegex(ValueError,'Completed fresh effect128'):inp.training(base)
            rows['metrics.json'].update(schema='worldline-action-cuda-fixed128-run-v1',completed_updates=127)
            with self.assertRaisesRegex(ValueError,'Completed fresh effect128'):inp.training(base)
            load.assert_not_called()

    def test_effect_contract_rejects_changed_loss_draws_and_heldout(self):
        contract=inp.read(inp.HERE/'training-contract.json')
        plan={**copy.deepcopy(contract),'source_sha256':inp.TRAINING_SOURCES}
        inp.validate_effect_contract(plan)
        changes=[]
        bad=copy.deepcopy(plan);bad['protocol']['auxiliary_weight']=.25;changes.append(bad)
        bad=copy.deepcopy(plan);bad['schedule'][100]['k']+=1;changes.append(bad)
        bad=copy.deepcopy(plan);bad['heldout_assessment']['manifest_sha256']='0'*64;changes.append(bad)
        bad=copy.deepcopy(plan);bad['checkpoint_selection']=True;changes.append(bad)
        bad=copy.deepcopy(plan);bad['source_sha256']['local']['effect.py']='0'*64;changes.append(bad)
        for bad in changes:
            with self.assertRaises(ValueError):inp.validate_effect_contract(bad)
