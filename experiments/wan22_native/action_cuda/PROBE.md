...
The [actual A100 numerical probe](results/a100-v1/README.md) completed both paired updates. Both pretraining native-versus-zero-adapter comparisons were bit-exact; second-update GRU gradients were nonzero and all 825 foundation values stayed unchanged. The [accepted preparation contract](PROBE-PLAN.md) fixes starts 0 then 8 and the separate parity check before training. It does not admit fixed16, image generation or an action-quality claim.
...
The separate [independent report](cpu-results/probe-independent-v1/report.json) passed five CPU cases against 61 unchanged source files, including its own test. It independently checks the dual parity bounds, exact two-update AdamW agreement with a direct future-flow objective, tolerated cross-length prefix behavior with unchanged targets, and pretraining failure order with retained native predictions. [Publication hashes](cpu-results/probe-independent-v1/publication.json) bind the unchanged report, review script and source snapshots. The published warning log replaces one private local workspace prefix with `<LOCAL_WORKSPACE>`; its raw and published hashes and replacement record are retained. These CPU checks are separate from the completed [actual CUDA audit](results/a100-v1/audits/probe/report.json).
