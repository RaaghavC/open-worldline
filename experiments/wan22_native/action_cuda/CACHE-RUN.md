...
The native FP32 CUDA VAE independently encodes starting observations and complete targets. Raw target latents are retained. Cross-length first-latent differences have predeclared maximum-absolute and relative-L2 limits of 1e-5. Same-length future-only perturbation and repeated single-image checks require exact equality. A failed check stops completion and retains partial evidence. The [actual A100 cache](results/a100-v1/README.md) completed all eight windows and seven observations; all eleven comparisons were bit-exact and the independent saved-data audit passed. This is data-validation evidence, not action-quality evidence.
