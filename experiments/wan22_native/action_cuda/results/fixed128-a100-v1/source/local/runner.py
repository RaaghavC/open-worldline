...
SCHEMA = 'worldline-action-cuda-fixed128-run-v1'
SCOPE = 'fixed128-fresh-cuda-training-only'
sha = evidence.sha
...
def sources():
    ...
    names=('prototype.py','runner.py','extension.py','test_prototype.py','test_runner.py','reference-identities.json')
...
def prepare(*, completed_probe, cache_run, text_directory, fixed16_run, diagnostic_prepared, cpu_report, profile, output):
    ...
    try:
        ...
        initial,prefix_schedule,prefix_draws,rng=extension.load_prefix(fixed16_run)
        initial,schedule,draws=prototype.fresh_inputs((1,*PROFILES[profile]['shape']),prefix_schedule=prefix_schedule,prefix_draws=prefix_draws,initial=initial)
        ...
        saved = {'initial-adapter.safetensors': initial, **dict(extension.chunks(draws)),
                 'positive.safetensors': {'context': context},
                 'initial-cpu-rng.safetensors': {'rng': rng}}
        artifacts = {}
        ...
        plan = {'schema': SCHEMA, 'status': 'prepared', 'scope': SCOPE, 'profile': profile,
                ...
                'objective':'Each fixed k506 branch loss must improve cp0 to cp16 before admission; after128 assess the same fixed draw and unchanged paired visual settings, using final128 only',
...
def admission(path, root, plan, diagnostic_record):
    ...
    expected = {'schema': 'worldline-action-cuda-fixed128-admission-v1', 'decision': 'admit',
                ...
                'diagnostic_evidence':extension.diagnostic_identity(diagnostic_record),'cfg_strategy_unchanged':True,'cpu_report_sha256':plan['cpu_report_sha256']}
...
def worker(config):
    ...
    try:
        ...
        report.update(status='passed', completed_updates=128, base_unchanged=True,
...
def main():
    ...
    for name in ('completed-probe','cache-run','text-directory','fixed16-run','diagnostic-prepared','diagnostic-result','cpu-report','output','prepared','weights','decision'):
    ...
    if args.worker_config:
        ...
        fields = ('prepared','completed_probe','cache_run','text_directory','diagnostic_result','weights','decision') if args.execute else (
            'completed_probe','cache_run','text_directory','fixed16_run','diagnostic_prepared','cpu_report','profile','output')
