...
ROOT=args.run_root.resolve();TRAIN=ROOT/'training';OUT=args.output.absolute()
if OUT.exists() or OUT.resolve().is_relative_to(ROOT) or OUT.resolve().is_relative_to(REPO):
    parser.error('Output must be fresh and outside both actual run and repository')
...
def arrays(path,keys=None):
    ...
    return values
OUT.mkdir(parents=True,exist_ok=False);began=time.monotonic();before_inventory=inventory()
(OUT/'audit.py').write_bytes(Path(__file__).read_bytes())
report={'schema':'worldline-action-cuda-fixed128-actual-independent-v1','status':'running','run':str(ROOT),
...
try:
    ...
    report['checks']['saved_draws']={'passed':True,'saved_noises':128,'regenerated_noises':0,'first16_file_sha256':ref['draw_file_sha256'],'first16_schedule_exact':True,'initial_cp0_exact':True}
        ...
        if step==0:
            ...
            for update in range(step-15,step+1):
                ...
                gradient_records.append({'update':update,'saved_post_clip_gradient_l2_fp64':actual_grad,'saved_gru_gradient_l2_fp64':actual_gru,
                    'reported_post_clip_gradient_l2':training['updates'][update-1]['gradient_l2_after_clip']})
    ...
    report.update(status='passed',completed_updates=128,foundation_values_unchanged=True,final_checkpoint_only=True,recovered_bytes_unchanged=True,recovered_files=len(before_inventory),recovered_bytes=sum(x['bytes']for x in before_inventory.values()),
