...
def check_runtime(root,plan,parent,worker,terminal):
    ...
    for row in samples:check_worker_sample(row)
    for row in parents:
    ...
    return {'passed':True,'worker_samples':len(samples),'parent_samples':len(parents),'parent_seconds':parent['elapsed_seconds'],
            ...
            'sequential_counter_mismatches':[{'sample_index':i,'seconds':r['seconds'],
                'cuda_reserved_bytes':r['cuda_reserved_bytes'],'cuda_allocated_bytes':r['cuda_allocated_bytes']}
                for i,r in enumerate(samples) if r['cuda_allocated_bytes']>r['cuda_reserved_bytes']],
            'counter_ordering_limitation':'The frozen monitor reads reserved bytes and then allocated bytes in separate calls. The shared row timestamp does not make them an instantaneous pair. Allocated<=reserved is not a declared runtime gate; all original caps remain independently enforced.'}
