...
PREP = BASE / 'work/wan22-action-cuda-visual-prepared-v2'
BASELINE = BASE / 'work/wan22-spatial-recovered-final-v1/recovered/spatial-results/clip-spatial-run-v1'
REVIEW = BASE / 'work/wan22-action-cuda-visual-independent-review-v2/report.json'
PLAN_SHA = '6feb229e4e27f2c44aa740184a1687bdad5b1ff06ada2eca1b4bb8d3b14e13f4'
REVIEW_SHA = '87aab83dece515fe1f7ae3496798376e8eee52f4814ee85a0285377866d85b23'
...
def audit(root, recovery, baseline, out):
    need(not (out/'report.json').exists(), 'fresh report required')
    began=time.monotonic(); before=inventory(root)
    report={'schema':'worldline-action-cuda-visual-actual-independent-v1','status':'running',
            'model_execution':False,'model_replay':False,'cloud_operations':False,
            'audit_source_sha256':sha(__file__),'reused_rgb_auditor_sha256':sha(RGB_PATH),'checks':{}}
    try:
        ...
        need(sha(REVIEW)==REVIEW_SHA and sha(root/'plan.json')==PLAN_SHA==sha(PREP/'plan.json'),
             'exact independently reviewed v2 plan')
        ...
        need(read(REVIEW)['source_sha256']==plan['source_sha256']['local'], 'reviewed actual source graph')
        cpu=read(root/'cpu-report.json')
        need(cpu['status']=='passed' and cpu['tests']==8 and cpu['sources_unchanged'] is True
             and cpu['source_sha256']==plan['source_sha256'] and cpu['cuda_initialized'] is False,
             'source-bound eight-case CPU report')
        ...
        required={'schema':'worldline-action-cuda-visual-admission-v1','decision':'admit','issued_by':'parent-agent',
                  ...
                  'fixed16_audit_sha256':'9c751fb0e7284edce0459740e203788fa1e371b691576c9eaa352d2b1c49cc54',
        ...
        for stage in ('core','decode'):
            ...
            need(set(actual)==set(trained) and type(actual['total_memory_bytes']) is int
                 and actual['total_memory_bytes']==85095874560 and trained['total_memory_bytes']==85093777408,
                 'actual observed +2 MiB capacity difference')
            need({k:v for k,v in actual.items() if k!='total_memory_bytes'}=={k:v for k,v in trained.items() if k!='total_memory_bytes'},
                 'all other actual native runtime fields equal')
            need(row['hardware_comparison']=={'policy':'All fields exact except reported total GPU bytes may be greater',
                 'training_total_memory_bytes':85093777408,'actual_total_memory_bytes':85095874560,'additional_reported_bytes':2097152},
        ...
        report['checks']['frozen_weights']={'passed':True,'core_tensors':825,'core_parameters':4999787712,
            ...
            'vae_tensors':196,'vae_parameters':704688668,'checkpoint16_sha256':plan['artifacts']['adapter.safetensors']}
        ...
        for arm in ('closed','open'):
            ...
            need(row['adapter']==plan['input_identity']['training']['adapter']
                 ...
                 and row['negative_context_adapter_training'] is False and row['quality_assessed'] is False,'actual adapter/CFG scope')
        ...
        report.update(status='passed',recovery_verified_sha256=sha(recovery),recovered_files=len(before),
...
if __name__=='__main__':
    ...
    audit(args.run_root.resolve(),args.recovery_verified.resolve(),args.baseline_root.resolve(),Path(__file__).resolve().parent)
