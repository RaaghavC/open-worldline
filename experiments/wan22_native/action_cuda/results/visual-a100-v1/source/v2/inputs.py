"""Bounded saved evidence and conditions for the first matched visual pair."""
import json
...
SCHEMA = 'worldline-action-cuda-visual-pair-v1'
SCOPE = 'first-trained-adapter-matched-wait-interact-visual-pair'
SAMPLER_SHA = '374e2d1b68015c7ce0b436dcddb68c6f5d8b247584feaa9140b94ec1aae3c314'
TRAINING_LOCAL = {
 'prototype.py':'ee5b9f15667a672331ee32c05b25bdd49208efb9328599f326a2ec40a477aba1',
 'runner.py':'b5c0b03565fb7c9d34c208658ea317d06075eaff5ad1171d1fadf26abab8ee27',
 'test_prototype.py':'17fae56a1ca94e4d4a96479aa542262d6cf4a3a54ab691a2ae7f4409521e8107',
 'test_runner.py':'a8a24734a0c0155c9251b285a0093017816283a5f4b332bda1ef6ec847ac6bbf',
}


def sources():
    repo = {**pe.source_hashes(independent=True), **se.sources()}
    local = {p:sha(HERE/p) for p in ('inputs.py','run.py','sampler.py','test_run.py')}
...
def training(directory):
    ...
    if (parent.get('schema')!='worldline-action-cuda-fixed16-run-v1' or parent.get('status')!='passed'
        or parent.get('completed_updates')!=16 or parent.get('model_execution') is not True
        or worker.get('status')!='passed' or worker.get('completed_updates')!=16 or worker.get('base_unchanged') is not True
        or result.get('status')!='passed' or result.get('completed_updates')!=16 or result.get('base_unchanged') is not True
        or result.get('training_forwards')!=32 or result.get('bridge_predictions')!=34
        ...
        raise ValueError('Completed fresh fixed16 spatial training required')
    expected={'repository':pe.source_hashes(independent=True),'local':TRAINING_LOCAL}
    ...
    if pointer.get('directory')!='checkpoint-0016' or pointer.get('completed_updates')!=16:
        raise ValueError('The last valid checkpoint must be exactly 16')
    folder=base/'training/checkpoint-0016'; manifest=read(file(folder,'manifest.json'))
    if (pointer.get('manifest_sha256')!=sha(folder/'manifest.json') or manifest.get('completed_updates')!=16
    ...
    return plan, folder/'adapter.safetensors', {'parent_sha256':sha(base/'metrics.json'),
        'worker_sha256':sha(base/'worker/metrics.json'),'training_sha256':sha(base/'training/metrics.json'),
...
def collect(training_run,cache_run,baseline_run):
    ...
    if trained['hardware']!=native['hardware']: raise ValueError('Baseline and training native runtime identities differ')
...
def prepare(training_run,cache_run,baseline_run,cpu_report,output):
    ...
    try:
        ...
        shutil.copyfile(checkpoint,out/'adapter.safetensors'); shutil.copyfile(cpu_report,out/'cpu-report.json')
        ...
        selections={'training':(training_run, ['metrics.json','plan.json','terminal.json','worker/metrics.json','worker/weight-load.json',
            'training/metrics.json','training/core-before.json','training/core-after.json','training/last-valid.json','training/checkpoint-0016/manifest.json']),
        ...
        plan={'schema':SCHEMA,'status':'prepared','scope':SCOPE,'profile':'spatial','arms':['closed','open'],
              ...
              'same_initial_observation_noise_text':True,'negative_context_adapter_training':False,
...
def read_prepared(directory):
    ...
    if (plan.get('schema')!=SCHEMA or plan.get('status')!='prepared' or plan.get('scope')!=SCOPE
        ...
        or plan.get('training') is not False): raise ValueError('Exact current matched visual plan required')
    ...
    links={'evidence/training/metrics.json':plan['input_identity']['training']['parent_sha256'],
           ...
           'evidence/training/training/checkpoint-0016/manifest.json':plan['input_identity']['training']['checkpoint_manifest_sha256'],
