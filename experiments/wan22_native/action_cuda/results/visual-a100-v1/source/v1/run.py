...
AUDIT_SHA='9c751fb0e7284edce0459740e203788fa1e371b691576c9eaa352d2b1c49cc54'
...
def admit(path,out,plan):
    ...
    required={'schema':'worldline-action-cuda-visual-admission-v1','decision':'admit','issued_by':'parent-agent',
        ...
        'fixed16_audit_sha256':AUDIT_SHA,'cpu_report_sha256':plan['cpu_report_sha256']}
...
def worker(config):
    ...
    try:
        ...
        if actual!=plan['input_identity']['training']['hardware']: raise ValueError('Native hardware/runtime differs from training')
        flags=probe._runtime_flags()
        report.update(plan_sha256=config['plan_sha256'],source_sha256=plan['source_sha256'],hardware=actual,runtime_flags=flags)
...
def main():
    ...
    for name in ('worker-config','training-run','cache-run','baseline-run','cpu-report','output','prepared','weights','decision'):
        parser.add_argument('--'+name,type=Path)
    args=parser.parse_args()
    if args.worker_config:return worker(inp.read(args.worker_config))
    names=('prepared','weights','decision') if args.execute else ('training_run','cache_run','baseline_run','cpu_report','output')
