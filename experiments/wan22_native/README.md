...
The [prepared CUDA reference](cuda_reference/README.md) uses byte-exact upstream model, attention, VAE and solver sources with original FP32 parameters. Its separate initial-pair and 50-step clip modes reuse the retained observation, noise and text. CPU tests check the execution protocol; CUDA installation, performance and image quality remain unmeasured. See the [cloud diagnostic plan](../../docs/cloud-gpu-diagnostic-2026-09-07.md) for the one-GPU target and provider-level cleanup requirements.
