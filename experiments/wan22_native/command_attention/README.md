...
The [512-update A100 training run](training/results/a100-v1/README.md) completed on a later lease and passed its saved-artifact audit. Door comparisons improved in the four-noise latent evaluation; camera comparisons stayed close to zero response. Generated-video evaluation is pending, so visible control has not been demonstrated. The [previous six-video experiment failed visible control](../intermediate_action/results/factorial-video-a100-v1/README.md). Architecture and objective changed together, with no claim of scientific novelty or attribution to either change alone.

The [first A100 resource profile](profile/results/a100-v1/README.md) completed 24 recorded exact zero-initialization comparisons and two optimizer updates. It correctly refused the 512-update run on that first pod because its forecast exceeded the remaining lease after recovery time was reserved. A new matching profile passed on the later training lease.
...
The completed fitting run used exactly 512 updates on the existing six camera/door sequences from one room. Two ordinary flow-matching branches run per update. Every fourth update adds one contrast from a fixed seven-pair cycle: three door comparisons and four camera-versus-stationary comparisons. Auxiliary future inputs contain shared Gaussian noise; future reference frames supply loss targets only. Both text-guidance branches receive the command.
...
The 1.75 GB input packet is retained with manifest hash `4f37fa59791d7b0645529e4b97978f08d928baf8987e79a9aad6da1b42f3046a`; its public release is pending. The [new training result](training/results/a100-v1/README.md) includes exact small records, five checkpoint manifests and the saved-artifact reader. The final checkpoint is `c3ece8c1b9c6f7eadedbd9db235ad63cab28b2c2ad2d097936c02f7efaf4393d`. Raw trained tensors remain outside Git, and their release has not been created. Door normalized contrast MSE falls to 0.8173–0.8991 from the zero-controller baseline of 1.0. Camera contrasts remain at 0.99968–1.00076 with cosine near zero. These are fixed-noise latent measurements in one seen room, not video quality results.
