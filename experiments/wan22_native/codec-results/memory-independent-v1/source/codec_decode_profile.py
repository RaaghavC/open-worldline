...
from wan22_native.codec_memory import cleanup_causal_convolutions
...
SHAPE=(1,48,5,18,32)
SOURCE_NAMES=tuple(INITIAL_SOURCES)+('codec_decode_profile.py','test_codec_decode.py','codec_memory.py','test_codec_memory.py')
...
def runtime_environment(device):
    return {'environment':{name:os.environ.get(name) for name in RUNTIME_VARIABLES},
        ...
        'record_only':True,'meaning':'Environment present before process startup; no variable is changed by this record'}
...
def validate_memory_cpu(path):
    report=json.loads(Path(path).read_text())
    if report.get('status')!='passed' or report.get('tests',0)<4:raise ValueError('Passed per-convolution cleanup CPU checks required')
    for name in ('codec.py','codec-source.json','vendor/vae2_2.py','codec_memory.py','test_codec_memory.py'):
        if report.get('source_sha256',{}).get(name)!=sha(HERE/name):raise ValueError('Cleanup source changed after CPU tests: '+name)
    if report.get('measurements',{}).get('full_17_frame_max_abs')!=0.:raise ValueError('Exact full17 CPU cleanup equality required')
    return sha(path)
...
def main():
    ...
    p.add_argument('--codec-cpu-report',type=Path,default=HERE/'codec-results/cpu-v3/tests.json')
    p.add_argument('--device',choices=('cpu','mps'),default='cpu')
    p.add_argument('--allocator-cleanup',action='store_true')
    p.add_argument('--per-convolution-cleanup',action='store_true')
    p.add_argument('--memory-cpu-report',type=Path,default=HERE/'codec-results/memory-cpu-v1/tests.json')
    args=p.parse_args()
    ...
    r.update(experiment='Synthetic full 17-frame Wan2.2 decoder cost diagnostic',device=args.device,dtype='float32',
        ...
        per_convolution_cleanup=args.per_convolution_cleanup,
    ...
    try:
        ...
        if args.per_convolution_cleanup:
            r['memory_cpu_sha256']=validate_memory_cpu(args.memory_cpu_report)
            shutil.copyfile(args.memory_cpu_report,args.output/'memory-cpu-tests.json')
        ...
        with (args.output/'per-convolution-cleanup.jsonl').open('x') as cleanup_log:
            def retain_cleanup(row):cleanup_log.write(json.dumps(row)+'\n');cleanup_log.flush()
            with chunk_timings(codec,args.device) as chunks,cleanup_temporal_chunks(codec) if args.allocator_cleanup else nullcontext() as counts,cleanup_causal_convolutions(codec,event_callback=retain_cleanup) if args.per_convolution_cleanup else nullcontext() as per_layer:
                r['per_convolution_cleanup_report']=per_layer
                video=guard.measure('decode_full_17_frames',lambda:codec.decode(latent))
        if args.per_convolution_cleanup and (not per_layer['hooks_removed'] or per_layer['cleanup_calls']<1 or per_layer['cleanup_calls']!=per_layer['completed_cleanups']):raise RuntimeError('Incomplete per-convolution cleanup')
        r['per_convolution_cleanup_log_sha256']=sha(args.output/'per-convolution-cleanup.jsonl')
        r['per_convolution_timing_scope']='Callback time includes synchronization waits for queued GPU work and empty_cache; not isolated allocator overhead'
