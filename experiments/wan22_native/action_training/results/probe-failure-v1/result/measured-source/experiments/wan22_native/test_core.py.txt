...
def test_native_profile_records_exact_runtime_without_changing_environment(device, monkeypatch):
    import os
    from . import profile_core as profile
    values = {"PYTORCH_MPS_LOW_WATERMARK_RATIO": "0.6",
    ...
    def recommended_memory():
        calls.append(True)
        return 24 * 2**30
    monkeypatch.setattr(profile.torch.mps, "recommended_max_memory", recommended_memory)
    before = dict(os.environ)
    report = profile.runtime_environment(device)
    assert report["environment"] == values
    assert report["recommended_max_memory_bytes"] == (24 * 2**30 if device == "mps" else None)
    assert calls == ([True] if device == "mps" else [])
    assert report["record_only"] is True and dict(os.environ) == before
