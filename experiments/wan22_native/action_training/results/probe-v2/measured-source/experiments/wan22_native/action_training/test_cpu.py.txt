...
class Tests(unittest.TestCase):
    ...
    def test_visual_admission_is_separate_mode_bound_and_hash_bound(self):
        ...
        with tempfile.TemporaryDirectory()as tmp:
            ...
            decision={'schema':'worldline-wan22-action-training-admission-v1','decision':'admit','issued_by':'parent-agent','mode':'probe',
                'scope':'optimizer-feasibility-only','foundation_visual_status':'failed',
    ...
    def test_numerical_probe_never_admits_fixed16_with_failed_visual_baseline(self):
        inputs={'cache_manifest_sha256':'c','roundtrip_metrics_sha256':'r'}
        with tempfile.TemporaryDirectory()as tmp:
            root=Path(tmp);visual=root/'visual.txt';visual.write_text('failed visual fixture')
            decision=dict(schema='worldline-wan22-action-training-admission-v1',decision='admit',issued_by='parent-agent',mode='probe',
                scope='optimizer-feasibility-only',foundation_visual_status='failed',source_sha256=r.source_hashes(),
                cache_manifest_sha256='c',roundtrip_metrics_sha256='r',visual_review='Numerical feasibility only',
                foundation_evidence=[dict(file='visual.txt',sha256=o.sha256(visual))])
            path=root/'decision.json';path.write_text(json.dumps(decision))
            self.assertEqual(r.validate_admission(path,'probe',inputs)['foundation_visual_status'],'failed')
            decision.update(mode='fixed16',scope='fixed16-action-pilot');path.write_text(json.dumps(decision))
            with self.assertRaises(ValueError):r.validate_admission(path,'fixed16',inputs)
            decision['foundation_visual_status']='passed';path.write_text(json.dumps(decision))
            self.assertEqual(r.validate_admission(path,'fixed16',inputs)['scope'],'fixed16-action-pilot')
            decision['scope']='optimizer-feasibility-only';path.write_text(json.dumps(decision))
            with self.assertRaises(ValueError):r.validate_admission(path,'fixed16',inputs)
    ...
    def test_pool_helper_bytes_are_bound_directly_by_snapshot_and_execution_gate(self):
        name='experiments/wan22_native/action_adapter/pooling.py'
        self.assertEqual(r.source_paths()[name],r.REPO/name)
        with tempfile.TemporaryDirectory()as tmp:
            root=Path(tmp);hashes=r.snapshot(root)
            self.assertEqual(hashes[name],o.sha256(root/'measured-source'/(name+'.txt')))
            report=root/'report.json';report.write_text(json.dumps(dict(status='passed',tests=17,
                source_sha256=hashes,source_unchanged_after_tests=True)))
            self.assertEqual(r.validate_cpu(report),o.sha256(report))
            original=r.sha256
            def changed(path):return '0'*64 if Path(path)==r.REPO/name else original(path)
            with mock.patch.object(r,'sha256',side_effect=changed):
                with self.assertRaises(ValueError):r.validate_cpu(report)
    ...
    def probe_fixture(self, root):
        result=root/'result';result.mkdir();bundle=result/'checkpoint-0002';bundle.mkdir()
        inputs={'cache_manifest_sha256':'cache','roundtrip_metrics_sha256':'roundtrip','text':{}}
        text=torch.ones(2,4096);inputs['text']['positive_tensor_sha256']=o.tensor_sha256(text)
        rows,draws=o.make_draws('probe');save_file(draws,str(root/'draws.safetensors'));save_file({'atrium':text},str(root/'positive.safetensors'))
        for name in ('adapter.safetensors','optimizer-and-rng.pt'):(bundle/name).write_bytes(b'opaque hash-only fixture')
        checkpoint={'completed_updates':2,'files':{n:o.sha256(bundle/n)for n in ('adapter.safetensors','optimizer-and-rng.pt')}}
        (bundle/'manifest.json').write_text(json.dumps(checkpoint))
        final=dict(checkpoint,directory='checkpoint-0002',manifest_sha256=o.sha256(bundle/'manifest.json'))
        for name in ('weight-load.json','core-before.json','core-after.json','last-valid.json'):(result/name).write_text('{}')
        updates=[{'command_gru_gradient_l2':float(i),'gradient_l2_before_clip':2.,'gradient_l2_after_clip':1.,'output_gradient_l2':.1,
                  'paired_mean_future_flow_mse':2.,'branches':[{'branch':'closed','future_flow_mse':1.},{'branch':'open','future_flow_mse':3.}]}for i in (0,1)]
        report={'status':'passed','protocol':r.protocol('probe'),'completed_updates':2,'source_sha256':r.source_hashes(),'input_evidence':inputs,
            'base_unchanged':True,'second_update_gru_gradient_nonzero':True,'updates':updates,'schedule':rows,
            'draw_file_sha256':o.sha256(root/'draws.safetensors'),'final_checkpoint':final,'initial_adapter_sha256':'fixture',
            'output_sha256':{n:o.sha256(result/n)for n in ('weight-load.json','core-before.json','core-after.json','last-valid.json')}}
        plan={'status':'complete','protocol':r.protocol('probe'),'source_sha256':r.source_hashes(),'input_evidence':inputs,'schedule':rows,
            'draw_file_sha256':o.sha256(root/'draws.safetensors'),'positive_file_sha256':o.sha256(root/'positive.safetensors')}
        (root/'plan.json').write_text(json.dumps(plan));(root/'terminal.json').write_text(json.dumps({'status':'complete','exit_code':0}))
        (result/'metrics.json').write_text(json.dumps(report))
        return inputs,report

    def test_probe_gate_rejects_nonfinite_norm_loss_and_incomplete_bundle(self):
        for mutation in ('inf_gru','nan_loss','missing_optimizer','wrong_update'):
            with self.subTest(mutation=mutation),tempfile.TemporaryDirectory()as tmp:
                root=Path(tmp);inputs,report=self.probe_fixture(root)
                self.assertFalse(r.validate_probe(root,inputs)['checkpoint_loaded_for_training'])
                if mutation=='inf_gru':report['updates'][1]['command_gru_gradient_l2']=float('inf')
                elif mutation=='nan_loss':report['updates'][0]['branches'][0]['future_flow_mse']=float('nan')
                else:
                    path=root/'result/checkpoint-0002/manifest.json';bundle=json.loads(path.read_text())
                    if mutation=='missing_optimizer':del bundle['files']['optimizer-and-rng.pt']
                    else:bundle['completed_updates']=1
                    path.write_text(json.dumps(bundle));report['final_checkpoint']['manifest_sha256']=o.sha256(path)
                (root/'result/metrics.json').write_text(json.dumps(report))
                with self.assertRaises(ValueError):r.validate_probe(root,inputs)

    def test_probe_gate_rejects_parent_draw_context_schedule_or_completion_changes(self):
        for mutation in ('draw','text','schedule','parent','watchdog'):
            with self.subTest(mutation=mutation),tempfile.TemporaryDirectory()as tmp:
                root=Path(tmp);inputs,_=self.probe_fixture(root)
                if mutation in ('draw','text'):(root/('draws.safetensors'if mutation=='draw'else'positive.safetensors')).write_bytes(b'corrupt')
                elif mutation=='watchdog':(root/'result/watchdog-stop.json').write_text('{}')
                else:
                    path=root/'plan.json';plan=json.loads(path.read_text())
                    if mutation=='schedule':plan['schedule'][0]['k']+=1
...
def main():
    ...
    report=dict(schema='worldline-wan22-action-training-cpu-v1',status='passed' if result.wasSuccessful()else'failed',tests=result.testsRun,
        ...
        dependencies={name:importlib.metadata.version(name)for name in ('torch','numpy','safetensors','diffusers','psutil')},
