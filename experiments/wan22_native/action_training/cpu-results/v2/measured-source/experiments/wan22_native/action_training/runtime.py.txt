...
from .objective import SEED, STARTS, OPTIMIZER, update_count
...
EXTRA=('experiments/wan22_native/action_adapter/__init__.py','experiments/wan22_native/action_adapter/model.py',
       'experiments/wan22_native/action_adapter/wrapper.py','experiments/wan22_native/codec_profile.py',
...
def validate_admission(path, mode, inputs):
    """A parent visual-review decision, separate from numerical/codec gates."""
    if path is None:raise ValueError('Explicit parent visual-foundation admission is missing')
    ...
    if (record.get('schema')!='worldline-wan22-action-training-admission-v1' or record.get('decision')!='admit'
        ...
        raise ValueError('Admission must bind this exact mode, sources, data and an explicit visual review')
    ...
    return dict(sha256=sha256(path),mode=mode,visual_review=record['visual_review'],foundation_evidence=evidence,
