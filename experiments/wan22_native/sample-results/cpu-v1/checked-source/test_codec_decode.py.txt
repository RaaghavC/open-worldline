...
class Tests(unittest.TestCase):
    def test_per_convolution_gate_requires_exact_current_full17_cpu_evidence(self):
        names=('codec.py','codec-source.json','vendor/vae2_2.py','codec_memory.py','test_codec_memory.py')
        report={'status':'passed','tests':4,'measurements':{'full_17_frame_max_abs':0.},'source_sha256':{name:sha(HERE/name) for name in names}}
        with tempfile.TemporaryDirectory() as directory:
            path=Path(directory)/'cpu.json';path.write_text(json.dumps(report));d.validate_memory_cpu(path)
            report['measurements']['full_17_frame_max_abs']=.0001;path.write_text(json.dumps(report))
            with self.assertRaises(ValueError):d.validate_memory_cpu(path)
            report['measurements']['full_17_frame_max_abs']=0.;report['source_sha256']['codec_memory.py']='changed';path.write_text(json.dumps(report))
            with self.assertRaises(ValueError):d.validate_memory_cpu(path)

    def test_runtime_environment_records_exact_values_without_changes(self):
        declared={'PYTORCH_MPS_LOW_WATERMARK_RATIO':'0.6','PYTORCH_ENABLE_MPS_FALLBACK':'0'}
        with mock.patch.dict(d.os.environ,declared,clear=True),mock.patch.object(torch.mps,'recommended_max_memory',side_effect=AssertionError('CPU metadata must not initialize MPS')):
            result=d.runtime_environment('cpu')
            self.assertEqual(result['environment'],{name:declared.get(name) for name in d.RUNTIME_VARIABLES})
            self.assertEqual(dict(d.os.environ),declared);self.assertIsNone(result['recommended_max_memory_bytes'])
        with mock.patch.dict(d.os.environ,declared,clear=True),mock.patch.object(torch.mps,'recommended_max_memory',return_value=123456):
            result=d.runtime_environment('mps')
            self.assertEqual(result['recommended_max_memory_bytes'],123456);self.assertTrue(result['record_only'])
