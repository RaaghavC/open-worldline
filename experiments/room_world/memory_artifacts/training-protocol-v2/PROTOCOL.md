# Fixed development protocol v2: recurrent memory for Room RGB prediction

Status: **512 updates per arm, three seeds, batched path on MPS**, fixed after the preceding 1,024-update attempt reached its time limit, before any quality evaluation. This amended protocol starts all six arms freshly; it uses no partial checkpoint. No learned-memory result, protected-test result or frontier-model comparison is established by this protocol.
...
AdamW settings are learning rate 0.0003, weight decay 0.0001, betas 0.9/0.999, epsilon 1e-8 and gradient-norm clipping at 1. The base remains frozen. Train **exactly 512 updates per arm** for all three seeds, using the explicit `batched` path. Every consecutive 32 updates is a shuffled pass over the same 32 training scenes. These 16 passes do not add independent scenes. Each arm therefore processes 1,024 branch sequences and 66,560 transitions; all six arms process 399,360 transitions in total. Initialize each seed freshly from the published base and its seeded zero-conditioning memory module. Do not warm-start from either profile. No checkpoint is selected by validation performance. All six arms must complete exactly 512 updates with the declared schedules, same initialization within seed, same optimizer and unchanged base. A failed or stopped arm leaves the matched study incomplete; do not compare a shorter surviving checkpoint as a completed study.
...
The preceding fixed 1,024-update attempt stopped in its first carried-state arm at 600.380 seconds, retaining 804 completed updates. No other arm or quality evaluation ran. Its first 512 recorded update intervals sum to 337.500 seconds. Those intervals include recovery writes but exclude later metrics writes and startup. The final 50 median interval was 0.849 seconds. This longer trace was slower than the earlier 50-update profile; the cause is not isolated.

This amendment selects 512 updates per arm, 16 complete passes over 32 original scene pairs. The six arms start from fresh initialization, with identical schedules within each seed. All optimizer, source, data, 600-second cap, memory guards, quality gates and checkpoint-selection rules are unchanged. There is no guarantee that all arms will finish within the cap. Another stopped arm leaves the study incomplete; no shorter checkpoint is eligible. The original protocol and complete stopped attempt remain published separately.

The previous MPS ordinary-L1 RGB-gradient equivalence check remains failed. Its separate common-upstream diagnostic isolates three changed residual signs in the tested carry example; it does not establish identical optimizer trajectories or reset-mode MPS equality. Both study arms use the same batched implementation.
...
../../work/atrium-env/bin/python -m experiments.room_world.memory_train \
  ...
  --output ../../work/room-memory-training-batched-512-v2 \
  --phase train --sequence-path batched --updates 512 \
...
../../work/atrium-env/bin/python -m experiments.room_world.memory_evaluate \
  --study ../../work/room-memory-training-batched-512-v2 \
  ...
  --output ../../work/room-memory-validation-batched-512-v2 \
  --device mps --max-seconds 600
```

The plan-only record is `work/room-memory-fixed512-plan-v2/study.json`; its schedule check verifies all three complete 512-entry schedules on CPU with zero optimizer updates. Profile checkpoints remain ineligible for evaluation. Do not change source while either actual run's source checks are active.
