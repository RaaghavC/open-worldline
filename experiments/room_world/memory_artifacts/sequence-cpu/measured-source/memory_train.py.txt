...
def run_arm(model, dataset, schedule, mode, output, *, seed, phase, learning_rate=3e-4,
            weight_decay=1e-4, check=None, base_checkpoint_sha256=BASE_SHA256):
    ...
    report = {"schema": "worldline-room-memory-arm-v1", "status": "running", "phase": phase,
        "mode": mode, "seed": seed, "requested_updates": len(schedule), "completed_updates": 0,
    ...
    try:
        ...
        for scene in schedule:
            ...
            loss = sequence_loss(model, rgb, actions, mode, check=check)
...
def _worker(config):
    ...
    result = run_arm(model, dataset, config["schedule"], config["mode"], config["output"],
            seed=config["seed"], phase=config["phase"], learning_rate=config["learning_rate"], weight_decay=config["weight_decay"])
...
def run_study(train, validation, output, *, phase="plan", updates=50, seeds=SEEDS, device="cpu", base=BASE,
              learning_rate=3e-4, weight_decay=1e-4, max_seconds=600):
    ...
    snapshot.mkdir()
    sources = {}
    for name in ("memory_train.py", "memory_evaluate.py", "memory_model.py", "memory_data.py", "model.py"):
    ...
    report = {"schema": "worldline-room-memory-study-v1", "status": "planned" if phase == "plan" else "running",
        "phase": phase, "requested_updates_per_arm": updates, "seeds": list(seeds), "arms": ["carry", "reset"],
        ...
        for seed in seeds:
            ...
            for mode in ("carry", "reset"):
                ...
                config = {"train": str(Path(train).resolve()), "train_manifest_sha256": training.manifest_sha256,
                    ...
                    "device": device, "source_sha256": sources, "schedule": report["schedules"][str(seed)], "learning_rate": learning_rate, "weight_decay": weight_decay}
                ...
                report["runs"].append({"seed": seed, "mode": mode, "terminal_status": terminal["status"],
                    "training_status": arm["status"], "completed_updates": arm["completed_updates"],
...
def main():
    ...
    result = run_study(args.train, args.validation, args.output, phase=args.phase, updates=args.updates if args.updates is not None else 50,
                       seeds=args.seeds, device=args.device, base=args.base, max_seconds=args.max_seconds)
