"""Read-only CPU artifact audit of the completed one-seed memory profile."""
import hashlib
import json
import math
...
ROOT, REPO = args.profile.resolve(), args.repo.resolve()
...
for name, expected in study['source_sha256'].items():
    assert sha(REPO / 'experiments/room_world' / name) == sha(ROOT / 'measured-source' / (name + '.txt')) == expected
...
for mode in ('carry', 'reset'):
    ...
    for state in recovery['optimizer_state_dict']['state'].values():
        assert state['step'].item() == 50
...
report = {'schema': 'worldline-room-memory-profile-independent-audit-v1', 'status': 'passed', 'device_used_for_audit': 'cpu',
    ...
    'limits': 'Profile only, one initialization seed and scheduled teacher-observed training scenes; no validation/control result or learned-memory claim. Memory values are sampled, not proven instantaneous peaks, and RSS/Metal values overlap.'}
