...
An untrained [recurrent memory addition](docs/room-rgb-experiment.md#recurrent-memory-component-not-yet-trained) now has explicit per-session state and 24,960 trainable parameters. Its [new development trajectories](experiments/room_world/memory_artifacts/data/README.md) contain 40 scenes with verified identical recent views before the door return. Correctness checks pass; learned memory gains remain unmeasured.
...
The [corrected foundation control](docs/native-wan-control.md) generated a coherent 17-frame doorway sequence at 512 × 288 in about 11 minutes using the attributed pretrained Wan model. It restores the native sampling recipe through a checked float32 local path. Initial-image and action conditioning still require separate evaluation.
