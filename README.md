...
The original [recurrent memory addition](docs/room-rgb-experiment.md#recurrent-memory-training-profile) has explicit per-session state and 24,960 trainable parameters. Its first matched 50-update training profile completed in about 73 seconds per arm. The [development trajectories](experiments/room_world/memory_artifacts/data/README.md) contain 40 scenes with verified identical recent views before the door return. Learned memory gains remain unmeasured.
...
The [corrected foundation control](docs/native-wan-control.md) generated a coherent 17-frame doorway sequence at 512 × 288 in about 11 minutes using the attributed pretrained Wan model. It restores the native sampling recipe through a checked float32 local path. A subsequent matched starting-image comparison reintroduced repeating texture and changed the room. The [image-conditioning design audit](docs/wan-image-conditioning-design.md) and untrained tokenwise-time prototype address how observed and noisy frames are identified. Reliable starting-image and action conditioning remain unresolved.
