...
The next candidate is a [4,936,448-parameter command-attention controller](../experiments/wan22_native/command_attention/README.md), replacing the single residual adapter with rank-32 query/key/value/output updates across blocks 24–29. Its prepared training objective includes both door and camera contrasts. The controller, fixed 512-update training runner and resource profiler have passed their small CPU checks. No actual GPU profile, training result or generated video exists for this candidate yet. It remains an implementation proposal for fitting one known scene, with no architecture-only advantage or novelty claim.
