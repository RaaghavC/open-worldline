...
[Author repository](https://github.com/asdfo123/ForgeWM), a 2026 project discovered in this search, reports a four-stage recipe on eight H20s, combining Matrix-Game 2/Wan2.1 lineage, open GF-Minecraft and Causal Forcing. Apache-2.0 code. Stages: bidirectional SFT 4k steps, teacher-forced causal AR 10k, consistency distillation 6k, DMD 2k. Data preprocessing reports 40k clips and approximately 89GB latent LMDB. Authors compare 352x640, four-step generation and note that input-action pitch conventions differ across GF and MG2. Claims similar quality to MG2 are author observations, not independent benchmark superiority. Underlying base and data licenses still apply. Exact initial release date was not established, so the latest repository must not be used to imply a dated priority claim. Confidence: medium, meaningful training code exists but not run here.
