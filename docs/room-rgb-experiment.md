...
The [trainer](../experiments/room_world/memory_train.py) now enforces matching initialization, data, schedules and completed budgets. The [evaluator](../experiments/room_world/memory_evaluate.py) preserves generated predictions, scores both return protocols and ordinary controls separately, and checks the independently recorded visible-door evidence. The evaluation excludes timing-profile checkpoints. The next fresh three-seed study will use 1,024 updates per arm, selected from the measured training cost before scoring image quality. Its protocol and source identities must be frozen before launch.
