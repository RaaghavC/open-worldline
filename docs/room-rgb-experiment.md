...
## Recurrent memory component, not yet trained
...
The caller initializes a separate zero state on each new episode and carries it using RGB and actions only. It must not receive the teacher's door state, camera pose or geometry. The implementation freezes the published predictor weights; the proposed training updates only the GRU and conditioning layer. This isolates the small memory addition; it may be insufficient to fix the baseline's image drift.
...
Nine CPU model tests passed, including exact equivalence to the original predictor at zero conditioning, finite GRU gradients after a conditioning update, a gradient path across 65 steps, reset-state removal of that path, independent cloned session states and unchanged frozen weights. Seven data tests verify intervention alignment, exact return-history aliasing, unsuccessful-interaction controls and exclusion of future RGB from a history. These are implementation checks. The component has not been trained, and the remaining trainer/evaluator protocol is still being prepared. Learned recurrent state, action-conditioned image prediction and diffusion objectives all have substantial prior art, including [World Models](https://arxiv.org/abs/1803.10122), [DIAMOND](https://arxiv.org/abs/2405.12399), [GameNGen](https://arxiv.org/abs/2408.14837) and [rectified flow](https://arxiv.org/abs/2209.03003). Success would establish a specific measured improvement in this project, not a new scientific method or frontier-model parity.
