...
Nine CPU model tests passed, including exact equivalence to the original predictor at zero conditioning, finite GRU gradients after a conditioning update, a gradient path across 65 steps, reset-state removal of that path, independent cloned session states and unchanged frozen weights. Seven data tests verify intervention alignment, exact return-history aliasing, unsuccessful-interaction controls and exclusion of future RGB from a history. These are implementation checks. The component has completed the timing profiles and the fixed six-arm training and validation study. Every memory gate failed; observed-history controls passed. Full results follow below. Learned recurrent state, action-conditioned image prediction and diffusion objectives all have substantial prior art, including [World Models](https://arxiv.org/abs/1803.10122), [DIAMOND](https://arxiv.org/abs/2405.12399), [GameNGen](https://arxiv.org/abs/2408.14837) and [rectified flow](https://arxiv.org/abs/2209.03003). Success would establish a specific measured improvement in this project, not a new scientific method or frontier-model parity.

## Fixed memory validation results

All six fresh arms completed 512 updates in 281.83 to 309.94 seconds per arm. The independently checked training records retain identical per-seed schedules and initial memory tensors, all completed optimizer steps, finite checkpoints and unchanged base weights. See the [complete training records](../experiments/room_world/memory_artifacts/training-512-v2/README.md).

The [completed validation](../experiments/room_world/memory_artifacts/validation-512-v2/README.md) failed every memory gate at hidden waits of 8, 16 and 32 steps. At the standard 16-step wait, the mean doorway-error reduction relative to resetting memory was −0.09% after an observed prefix and 0.61% during uninterrupted generation. Both branches were correctly distinguished in 0% and 8.33% of scene/seed pairs, respectively. These are well below the fixed 30% error-reduction and 80% pair-correctness thresholds.

Every observed-history control type and initialization seed retained one-step accuracy within 5% of both matched reset and frozen references. This is a separate control result, not successful memory or long generated-history control. The complete presentation preserves 48 videos and 288 contact pages without selecting by quality. All generated tensors, including controls and variable waits, remain in the full evaluation archive. Reserved test scenes were not opened.
