...
The subsequent [image-conditioning source audit](wan-image-conditioning-design.md) identifies a concrete difference: official Wan2.2 TI2V assigns time zero to observed tokens and the current noise time to future tokens. The tested Wan2.1 clamp broadcasts the current noise time to both. A separate [untrained tokenwise-time extension](../experiments/wan_adapter/tokenwise_time/README.md) preserves the existing 1.3B parameter layout. Its CPU checks cover uniform-time equality and mixed-time indexing and modulation. It has no trained image-conditioning or generated-quality result yet.
