...
A separate narrow training question is whether a small carried recurrent state improves the explicit door-memory test relative to a parameter-matched state-reset control. The [frozen proposal](room-rgb-experiment.md#smallest-next-memory-experiment-not-implemented) specifies fresh scene splits, three seeds and acceptance thresholds. Existing inspected test scenes are now development evidence. This can test memory in a narrow domain; it cannot establish frontier graphics or novelty.
