...
A separate narrow training question is whether a small carried recurrent state improves the explicit door-memory test relative to a parameter-matched state-reset control. The [memory study](room-rgb-experiment.md#recurrent-memory-component-not-yet-trained) specifies fresh scene splits, three seeds and provisional acceptance thresholds. Its untrained recurrent component and development data now exist; trainer/evaluator preparation and protocol freeze remain before reserved-test access. Existing inspected test scenes are now development evidence. This can test memory in a narrow domain; it cannot establish frontier graphics or novelty.

The subsequent [Atrium adapter study](wan-atrium-pilot.md) established a real 512 × 288 training path through a frozen Wan foundation on this Mac. The original adapter completed ten updates, but generated videos failed camera control and scored worse than repeating the starting frame. Full-precision decoding retained the artifacts. A [native sampling audit](wan-native-sampler-audit.md) found several changes from the official pretrained recipe, including image clamping, solver settings, negative text and numerical precision. The immediate next step is a numerically checked pure-T2V control. More adapter training is not justified by the present results alone.
