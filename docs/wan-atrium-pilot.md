...
These differences prevent the failed videos from establishing native Wan's quality. A subsequent [numerically checked pure-T2V control](native-wan-control.md) completed with the official scheduler and produced a coherent doorway sequence without the prior lattice. It generated all 17 frames from noise in about 11 minutes. This establishes a working external-foundation path at the tested shape, not an original action-conditioned world model. Adding only the initial-image clamp to that successful configuration is the next proposed comparison before more adapter training.
