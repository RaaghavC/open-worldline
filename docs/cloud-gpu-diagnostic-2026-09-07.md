# Prepared GPU reference experiment

Checked September 7, 2026. No cloud instance has been created and no CUDA model result has been measured. The [prepared reference runner](../experiments/wan22_native/cuda_reference/README.md) tests whether the failed local Wan2.2 video also fails with the original CUDA model and FlashAttention 2. This is an external-model diagnostic, not original model training or evidence of Genie 3 parity.
...
| --- | ---: | --- |
| RunPod Secure Cloud, A100 80 GB PCIe or SXM | $1.59 | Cheapest qualifying listed option; actual availability and deployment quote unverified. |
...
The official CLI 2.12.0 was downloaded locally and its Darwin ARM64 binary matched both the GitHub asset digest and the published checksum. Its version command returned `runpodctl 2.12.0-51ca7f0`. Only local version/help commands were executed; no authenticated provider operation has been tested. The verified binary SHA256 is `ea5d936c0d9df23f7b2ff667480cb5a8344ac29b7c769751182c6180f4277408`. [Official release](https://github.com/runpod/runpodctl/releases/tag/v2.12.0), [retained download identities](cloud-gpu-diagnostic-evidence/runpodctl-download-proof.json)
...
## Required evidence from an eventual run

Retain the live price and hardware selection, creation and deletion times, setup log, installed package versions, GPU and compiler reports, source-bound CPU checks, and the runner's complete or partial artifacts. Compare the initial predictions before interpreting clip quality. Report setup time separately from model loading, prediction and decoding. The work is still preparation until these records exist.
